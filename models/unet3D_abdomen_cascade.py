...
class UpConvBlock(nn.Module):
    def __init__(self, in_channels, res_channels=0, last_layer=False, num_classes=None):
        ...
        self.relu = nn.ReLU()
        self.bn = nn.BatchNorm3d(num_features=res_channels)
        self.conv1 = nn.Conv3d(in_channels=in_channels+res_channels, out_channels=res_channels, kernel_size=3, padding=1)
        self.conv2 = nn.Conv3d(in_channels=res_channels, out_channels=res_channels, kernel_size=3, padding=1)
        self.last_layer = last_layer
        if last_layer:
            self.conv3 = nn.Conv3d(in_channels=res_channels, out_channels=num_classes, kernel_size=1)
...
class cascade_3DUNet(nn.Module):
    def __init__(self, in_channels, num_classes, level_channels=[64, 128, 256], bottleneck_channel=256):
        super(cascade_3DUNet, self).__init__()
        ...
        self.s_block3 = UpConvBlock(in_channels=bottleneck_channel, res_channels=level_3_chnls)

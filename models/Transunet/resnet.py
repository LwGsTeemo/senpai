...
class PreActBottleneck3d(nn.Module):
    """Pre-activation (v2) bottleneck block for 3D data.
    """
    def __init__(self, cin, cout=None, cmid=None, stride=1):
        ...
        self.conv2 = conv3x3x3(cmid, cmid, stride, bias=False)
...
class ResNetV2_3d(nn.Module):
    """Implementation of Pre-activation (v2) ResNet mode for 3D data."""
    def __init__(self, block_units, width_factor):
        ...
        self.root = nn.Sequential(OrderedDict([
            ('conv', StdConv3d(1, width, kernel_size=7, stride=2, bias=False, padding=3)),
            ('gn', nn.GroupNorm(32, width, eps=1e-6)),
            ('relu', nn.ReLU(inplace=True)),
            # ('pool', nn.MaxPool3d(kernel_size=3, stride=2, padding=0))
        ]))

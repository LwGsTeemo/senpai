...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, bottleneck=False) -> None:
        super(ConvBlock, self).__init__()
        self.encode = nn.Sequential(
            nn.Conv3d(in_channels=in_channels, out_channels=out_channels//2, kernel_size=kernel_size, padding=padding),
            nn.BatchNorm3d(num_features=out_channels//2),
            nn.ReLU(),
            nn.Conv3d(in_channels=out_channels//2, out_channels=out_channels, kernel_size=kernel_size, padding=padding),
        ...
        if not bottleneck:
            ...
            self.pooling4 = nn.MaxPool3d(kernel_size=4, stride=4)
            self.pooling8 = nn.MaxPool3d(kernel_size=8, stride=8)
        
        for m in self.children():
            init_weights(m, init_type='kaiming')
    
    def forward(self, x, cut4=None, cut8=None):
        if cut4 is not None:
            x = torch.cat((x, cut4), dim=1)
        if cut8 is not None:
            x = torch.cat((x, cut8), dim=1)
        res = self.encode(x)
        out = None
        if not self.bottleneck:
            out = self.pooling2(res)
            cut4 = self.pooling4(res)
            cut8 = self.pooling8(res)
        else:
            out = res
        return out, res, cut4, cut8
...
class UNet_revised(nn.Module):
    def __init__(self, in_channels, num_classes, level_channels=[64, 128, 256, 512], bottleneck_channel=1024, deep_supervision=True):
        super(UNet_revised, self).__init__()
        ...
        self.a_block3 = ConvBlock(in_channels=level_2_chnls+level_1_chnls, out_channels=level_3_chnls)
        self.a_block4 = ConvBlock(in_channels=level_3_chnls+level_2_chnls+level_1_chnls, out_channels=level_4_chnls)
        self.bottleNeck = ConvBlock(in_channels=level_4_chnls+level_3_chnls+level_2_chnls, out_channels=bottleneck_channel, bottleneck=True)
    ...
    def forward(self, input):
        out, residual_level1, cut4_l1, cut8_l1 = self.a_block1(input)
        out, residual_level2, cut4_l2, cut8_l2 = self.a_block2(out)
        out, residual_level3, cut4_l3, _ = self.a_block3(out, cut4_l1)
        out, residual_level4, _, _ = self.a_block4(out, cut8_l1, cut4_l2)
        out, _, _, _ = self.bottleNeck(out, cut8_l2, cut4_l3)
